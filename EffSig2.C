...
const double ranges[][2] = {{0,3.15},    {0,2.5}    ,     {0,1}  ,     {0,1},       {0.05,1},   {0,4.5},    {0,2},  {0.5, 10} , {0.5, 10},   {0,85},                       {0,85}   ,         {-22,22}  ,          {-22,22}};
...
for (int ifile = 0; ifile < sizeof(files)/sizeof(files[0]); ++ifile) {
    ...
    for (int i = 0; i < nVars; ++i) {
        ...
        if (path_to_file.Contains("Bs")){ 
            ...
            treedata->Draw(Form("%s >> hist_BKG", var.Data()), Form("Balpha<0.173754 && Bnorm_svpvDistance_2D>4.473 && ((Bmass < 5.24188) || (Bmass > 5.49188)) && %s && %s", cut.Data(), sepcCASES.Data()));
        } else if (path_to_file.Contains("Bd")){ 
        ...
        if (path_to_file.Contains("Bd")){ 
	      ...
	      treedata->Draw(Form("%s >> hist_BKG"     , var.Data()), Form("Bchi2cl>0.003 && ((Bmass < 5.17807) || (Bmass > 5.38111)) && %s && %s", cut.Data(), sepcCASES.Data()));       // BKG -- (notice the *!* in the first %s)
...
// --- Pick point: minimal FPR with TPR >= (1 - maxSignalLoss); fallback = closest to (0,1)
const double maxSignalLoss = 0.05;
const double tprMinRight   = 1.0 - maxSignalLoss;

double minDistRight = 999.0;
int bestBinRight = -1;
double bestFprRight = 1e9;

for (int i = 0; i < nbins; ++i) {
    double fpr = effBkg_right[i];
    double tpr = effSig_right[i];
    if (tpr >= tprMinRight && fpr < bestFprRight) {
        bestFprRight = fpr;
        bestBinRight = i;
    }
}

// If no point found with TPR >= (1 - maxSignalLoss), fallback to closest to (0,1)
if (bestBinRight < 0) { // fallback
    double minDistRight = 1e9;
    for (int i = 0; i < nbins; ++i) {
        double fpr = effBkg_right[i];
        double tpr = effSig_right[i];
        double dist = std::sqrt(fpr*fpr + (1.0 - tpr)*(1.0 - tpr));
...
double cutValueRight = histogram->GetBinLowEdge(bestBinRight + 1);
// Output the best cut point
if (bestBinRight >= 0) {
    double cutValueRight = histogram->GetBinLowEdge(bestBinRight + 1); // +1 because bins start at 1 in ROOT
    std::cout << "\n[RIGHT CUT] Best ROC Point (Closest to (0,1)) or TPR ≥:" << (1.0 - maxSignalLoss) << std::endl;
...
auto legend_r = new TLegend(0.6, 0.75, 0.88, 0.88);
    ...
    legend_r->AddEntry((TObject*)0,Form("Cut: %s > x", var.Data()));
    legend_r->AddEntry((TObject*)0, Form("AUC = %.4f", aucRight));
    legend->AddEntry((TObject*)0, Form("FPR = %.4f, TPR = %.4f", effBkg_right[bestBinRight], effSig_right[bestBinRight]), "");
...
// --- Pick point: minimal FPR with TPR >= (1 - maxSignalLoss); fallback = closest to (0,1)
const double tprMinLeft = 1.0 - maxSignalLoss;

double bestFprLeft = 1e9;
double minDistLeft = 999.0;
int bestBinLeft = -1;

// Find the best point based on TPR >= (1 - maxSignalLoss)
for (int i = 0; i < nbins; ++i) {
    double fpr = effBkg_left[i];
    double tpr = effSig_left[i];
    if (tpr >= tprMinLeft && fpr < bestFprLeft) {
        bestFprLeft = fpr;
        bestBinLeft = i;
    }
}
...
if (bestBinLeft < 0) { // fallback
    double minDistLeft = 1e9;
    for (int i = 0; i < nbins; ++i) {
        double fpr = effBkg_left[i];
        double tpr = effSig_left[i];
        double dist = std::sqrt(fpr*fpr + (1.0 - tpr)*(1.0 - tpr));
...
if (bestBinLeft >= 0) {
    double cutValueLeft = histogram->GetBinLowEdge(bestBinLeft + 1);
    std::cout << "\n[LEFT CUT] Best ROC Point (Closest to (0,1)) or TPR ≥:" << (1.0 - maxSignalLoss) << std::endl;
...
auto legend = new TLegend(0.6, 0.75, 0.88, 0.88);
    ...
    legend->AddEntry((TObject*)0,Form("Cut: %s < x", var.Data()));
